...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 menuButtonSizeXY = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);

    const ImRect bb(pos, pos + menuButtonSizeXY);
    ItemSize(menuButtonSizeXY, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
...
bool ImGui::InvisibleButton(const char* str_id, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 menuButtonSizeXY = CalcItemSize(size_arg, 0.0f, 0.0f);
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + menuButtonSizeXY);
...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 menuButtonSizeXY, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ItemSize(menuButtonSizeXY, (menuButtonSizeXY.y >= default_size) ? g.Style.FramePadding.y : -1.0f);
    if (!ItemAdd(bb, id))
        return false;
    ...
    RenderArrow(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (menuButtonSizeXY.x - g.FontSize) * 0.5f), ImMax(0.0f, (menuButtonSizeXY.y - g.FontSize) * 0.5f)), text_col, dir);
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& menuButtonSizeXY, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;

    ImRect bb(window->DC.CursorPos, window->DC.CursorPos + menuButtonSizeXY);
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& menuButtonSizeXY, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + menuButtonSizeXY + padding * 2);
    ItemSize(bb);
    if (!ItemAdd(bb, id))
        return false;
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& menuButtonSizeXY, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    return ImageButtonEx(id, user_texture_id, menuButtonSizeXY, uv0, uv1, padding, bg_col, tint_col);
}
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 menuButtonSizeXY = CalcItemSize(size_arg, CalcItemWidth(), g.FontSize + style.FramePadding.y * 2.0f);
    ImRect bb(pos, pos + menuButtonSizeXY);
    ItemSize(menuButtonSizeXY, style.FramePadding.y);
    if (!ItemAdd(bb, 0))
        return;
...
void ImGui::Dummy(const ImVec2& menuButtonSizeXY)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& menuButtonSizeXY, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + menuButtonSizeXY);
...
bool ImGui::VSliderFloat(const char* label, const ImVec2& menuButtonSizeXY, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return VSliderScalar(label, menuButtonSizeXY, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}

bool ImGui::VSliderInt(const char* label, const ImVec2& menuButtonSizeXY, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
    return VSliderScalar(label, menuButtonSizeXY, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
...
bool ImGui::InputTextMultiline(const char* label, char* buf, size_t buf_size, const ImVec2& menuButtonSizeXY, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    return InputTextEx(label, NULL, buf, (int)buf_size, menuButtonSizeXY, flags | ImGuiInputTextFlags_Multiline, callback, user_data);
}
...
static void    STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, ImGuiInputTextState* obj, int line_start_idx)
{
    ...
    const ImVec2 menuButtonSizeXY = InputTextCalcTextSizeW(text + line_start_idx, text + obj->CurLenW, &text_remaining, NULL, true);
    r->x0 = 0.0f;
    r->x1 = menuButtonSizeXY.x;
    r->baseline_y_delta = menuButtonSizeXY.y;
    r->ymin = 0.0f;
    r->ymax = menuButtonSizeXY.y;
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 menuButtonSizeXY)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (menuButtonSizeXY.x == 0.0f)
        menuButtonSizeXY.x = default_size;
    if (menuButtonSizeXY.y == 0.0f)
        menuButtonSizeXY.y = default_size;
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + menuButtonSizeXY);
    ItemSize(bb, (menuButtonSizeXY.y >= default_size) ? g.Style.FramePadding.y : 0.0f);
    if (!ItemAdd(bb, id))
        return false;
    ...
    float grid_step = ImMin(menuButtonSizeXY.x, menuButtonSizeXY.y) / 2.99f;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec2 menuButtonSizeXY(size_arg.x != 0.0f ? size_arg.x : label_size.x, size_arg.y != 0.0f ? size_arg.y : label_size.y);
    ImVec2 pos = window->DC.CursorPos;
    pos.y += window->DC.CurrLineTextBaseOffset;
    ItemSize(menuButtonSizeXY, 0.0f);
    ...
    if (size_arg.x == 0.0f || (flags & ImGuiSelectableFlags_SpanAvailWidth))
        menuButtonSizeXY.x = ImMax(label_size.x, max_x - min_x);

    // Text stays at the submission position, but bounding box may be extended on both sides
    const ImVec2 text_min = pos;
    const ImVec2 text_max(min_x + menuButtonSizeXY.x, pos.y + menuButtonSizeXY.y);
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    ImVec2 menuButtonSizeXY = ImFloor(CalcItemSize(size_arg, CalcItemWidth(), GetTextLineHeightWithSpacing() * 7.25f + style.FramePadding.y * 2.0f));
    ImVec2 frame_size = ImVec2(menuButtonSizeXY.x, ImMax(menuButtonSizeXY.y, label_size.y));
...
bool ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items)
{
    ...
    ImVec2 menuButtonSizeXY;
    menuButtonSizeXY.x = 0.0f;
    menuButtonSizeXY.y = GetTextLineHeightWithSpacing() * height_in_items_f + g.Style.FramePadding.y * 2.0f;
    return BeginListBox(label, menuButtonSizeXY);
}
#endif
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
    ...
    ImVec2 menuButtonSizeXY(0.0f, ImFloor(GetTextLineHeightWithSpacing() * height_in_items_f + g.Style.FramePadding.y * 2.0f));

    if (!BeginListBox(label, menuButtonSizeXY))
        return false;
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
    ...
    if (bar_window == NULL || bar_window->BeginCount == 0)
    {
        ...
        ImVec2 menuButtonSizeXY = avail_rect.GetSize();
        menuButtonSizeXY[axis] = axis_size;
        SetNextWindowPos(pos);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    ImVec2 menuButtonSizeXY = TabItemCalcSize(label, p_open != NULL);
    ...
    if (tab == NULL)
    {
        ...
        tab->ID = id;
        tab->Width = menuButtonSizeXY.x;
        tab_bar->TabsAddedNew = true;
        tab_is_new = true;
    }
    tab_bar->LastTabItemIdx = (ImS16)tab_bar->Tabs.index_from_ptr(tab);
    tab->ContentWidth = menuButtonSizeXY.x;
    ...
    else
    {
        ...
        tab->NameOffset = (ImS32)tab_bar->TabsNames.menuButtonSizeXY();
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button)
{
    ImGuiContext& g = *GImGui;
    ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 menuButtonSizeXY = ImVec2(label_size.x + g.Style.FramePadding.x, label_size.y + g.Style.FramePadding.y * 2.0f);
    if (has_close_button)
        menuButtonSizeXY.x += g.Style.FramePadding.x + (g.Style.ItemInnerSpacing.x + g.FontSize); // We use Y intentionally to fit the close button circle.
    else
        menuButtonSizeXY.x += g.Style.FramePadding.x + 1.0f;
    return ImVec2(ImMin(menuButtonSizeXY.x, TabBarCalcMaxTabWidth()), menuButtonSizeXY.y);
}
